...
import { loadData, loadSections, assembleHomepage, buildSimplePage, applyTextEdits, applyAttrEdits, fixContactForm, fixContactAreaSelect, fixPlaybookForm, trimToFirstSectionClose, escapeHtml, renderImage, stylesheetLinks, asset, SECTION_IDS, FOOTER_TEXT_KEYS } from './src/render.mjs';
...
function step3b_emitAdminSchema() {
  ...
  const legacySections = [
    ...
    { id: 'hero', label: 'Hero, nav & ticker', hooks: scanHooks(preludeBody) },
    ...SECTION_IDS.map((id) => {
      const file = resolve(sectionDir, id + '.html');
      const body = existsSync(file) ? readFileSync(file, 'utf8') : '';
      return { id, label: id.charAt(0).toUpperCase() + id.slice(1), hooks: scanHooks(body) };
    }),
    ...
    { id: 'footer', label: 'Footer', hooks: FOOTER_TEXT_KEYS }
  ];

  const canvas = 'window.FB_CANVAS = ' + JSON.stringify({
    types, fontFamilies: FONT_FAMILIES, themeColors: THEME_COLORS,
    breakpoints: BREAKPOINTS, designWidth: DESIGN_WIDTH, legacySections
  }, null, 2) + ';\n';
