...
import { loadData, loadSections, assembleHomepage, buildSimplePage, applyTextEdits, fixContactForm, fixPlaybookForm, trimToFirstSectionClose, escapeHtml, renderImage, stylesheetLinks, asset, SECTION_IDS } from './src/render.mjs';
...
function step3b_emitAdminSchema() {
  ...
  const legacySections = SECTION_IDS.map((id) => {
    const file = resolve(sectionDir, id + '.html');
    const body = existsSync(file) ? readFileSync(file, 'utf8') : '';
    const hooks = [...body.matchAll(/data-(?:edit|img)="([^"]+)"/g)].map((m) => m[1]);
    return { id, label: id.charAt(0).toUpperCase() + id.slice(1), hooks: [...new Set(hooks)] };
  });
