...
function step11b_privacyPage(content, prelude) {
  ...
  body += '<p>The enrollment form also asks, optionally, whether you enrolled because you enjoyed the website. It is a yes or no we keep on your registration. It changes nothing about what you pay or what your athlete receives; it tells Coach Blake how families are finding him, and it sets the fee the site\'s developer is paid out of Coach Blake\'s own share. Leaving it blank is a no, and that is fine.</p>\n';
...
function step11d_enrollPages(sections, content, prelude) {
  ...
  // required defaults true: the two agreement boxes are contract and must stay required. The
  // attribution box is the one optional checkbox on the form, so it opts out.
  const check = (id, name, html, required = true) => '<div class="fld">\n<label for="' + id + '" style="' + LABEL_CSS + '">\n' +
    '<input type="checkbox" id="' + id + '" name="' + name + '" value="yes"' + (required ? ' required' : '') + ' style="' + BOX_CSS + '">\n<span>' + html + '</span>\n</label>\n</div>\n';
  ...
  // The one OPTIONAL box on the form. Ticking it changes nothing a family pays or receives; it
  // tells Coach Blake the site is what brought them in. features.css celebrates the tick.
  body += check('enLikedSite', 'likedSite', 'Did you enroll because you enjoyed the website? <span class="en-opt">Optional</span>', false);
